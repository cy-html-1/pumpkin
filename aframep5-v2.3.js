...
AFRAME.registerComponent('generic-interaction-handler', {
    ...
    // 2024-11-10: Added support for object-specific tick functions
    tick: function () {
        if (this.el.eRef.tickFunction) {
            this.el.eRef.tickFunction(this.el.eRef);
        }
    }
});
...
AFRAME.registerComponent('vrcontroller-righthand', {
    ...
    init: function () {
        ...
        el.addEventListener('triggerdown', function (e) {
            AFrameP5.controller_rightTriggerDown = true;
        });

        el.addEventListener('triggerup', function (e) {
            AFrameP5.controller_rightTriggerDown = false;
        });

        el.addEventListener('gripdown', function (e) {
            AFrameP5.controller_rightGripDown = true;
        });

        el.addEventListener('gripup', function (e) {
            AFrameP5.controller_rightGripDown = false;
        });

        el.addEventListener('abuttondown', function (e) {
            AFrameP5.controller_aButtonDown = true;
        });

        el.addEventListener('abuttonup', function (e) {
            AFrameP5.controller_aButtonDown = false;
        });

        el.addEventListener('bbuttondown', function (e) {
            AFrameP5.controller_bButtonDown = true;
        });

        el.addEventListener('bbuttonup', function (e) {
            AFrameP5.controller_bButtonDown = false;
        });

        el.addEventListener('thumbstickmoved', function (e) {
            AFrameP5.controller_rightThumbstickRawData = {
                x: e.detail.x,
                y: e.detail.y
            };
            if (e.detail.y > 0.95) {
                AFrameP5.controller_rightThumbstickDirection = "DOWN";
            }
            else if (e.detail.y < -0.95) {
                AFrameP5.controller_rightThumbstickDirection = "UP";
            }
            else if (e.detail.x < -0.95) {
                AFrameP5.controller_rightThumbstickDirection = "LEFT";
            }
            else if (e.detail.x > 0.95) {
                AFrameP5.controller_rightThumbstickDirection = "RIGHT";
            }
            else {
                AFrameP5.controller_rightThumbstickDirection = false;
            }
        });
    }
});
...
AFRAME.registerComponent('vrcontroller-lefthand', {
    ...
    init: function () {
        ...
        el.addEventListener('triggerdown', function (e) {
            AFrameP5.controller_leftTriggerDown = true;
        });

        el.addEventListener('triggerup', function (e) {
            AFrameP5.controller_leftTriggerDown = false;
        });

        el.addEventListener('gripdown', function (e) {
            AFrameP5.controller_leftGripDown = true;
        });

        el.addEventListener('gripup', function (e) {
            AFrameP5.controller_leftGripDown = false;
        });

        el.addEventListener('xbuttondown', function (e) {
            AFrameP5.controller_xButtonDown = true;
        });

        el.addEventListener('xbuttonup', function (e) {
            AFrameP5.controller_xButtonDown = false;
        });

        el.addEventListener('ybuttondown', function (e) {
            AFrameP5.controller_yButtonDown = true;
        });

        el.addEventListener('ybuttonup', function (e) {
            AFrameP5.controller_yButtonDown = false;
        });

        el.addEventListener('thumbstickmoved', function (e) {
            AFrameP5.controller_leftThumbstickRawData = {
                x: e.detail.x,
                y: e.detail.y
            };
            if (e.detail.y > 0.95) {
                AFrameP5.controller_leftThumbstickDirection = "DOWN";
            }
            else if (e.detail.y < -0.95) {
                AFrameP5.controller_leftThumbstickDirection = "UP";
            }
            else if (e.detail.x < -0.95) {
                AFrameP5.controller_leftThumbstickDirection = "LEFT";
            }
            else if (e.detail.x > 0.95) {
                AFrameP5.controller_leftThumbstickDirection = "RIGHT";
            }
            else {
                AFrameP5.controller_leftThumbstickDirection = false;
            }
        });
...
AFRAME.registerComponent('rotation-and-position-reader', {
    ...
    tick: function () {
        // nudge position (centralized to handle this here to support multiple key presses for custom movement)
        if (AFrameP5.camera_nudge_dirty) {

            // only need to update THREE.JS object
            AFrameP5.world_instance_reference.camera.holder.object3D.position.x = AFrameP5.camera_positionWorld.x + AFrameP5.camera_nudge_values.x;		// this works, but breaks when WASD is used
            AFrameP5.world_instance_reference.camera.holder.object3D.position.y = AFrameP5.camera_positionWorld.y + AFrameP5.camera_nudge_values.y;
            AFrameP5.world_instance_reference.camera.holder.object3D.position.z = AFrameP5.camera_positionWorld.z + AFrameP5.camera_nudge_values.z;

            // fix: reset camera back to its initial origin point
            AFrameP5.world_instance_reference.camera.cameraEl.object3D.position.x = 0;
            AFrameP5.world_instance_reference.camera.cameraEl.object3D.position.y = 0;
            AFrameP5.world_instance_reference.camera.cameraEl.object3D.position.z = 0;
            
            // reset nudge values & turn off dirty flag
            AFrameP5.camera_nudge_values.x = 0;
            AFrameP5.camera_nudge_values.y = 0;
            AFrameP5.camera_nudge_values.z = 0;
            AFrameP5.camera_nudge_dirty = false;
        }

        // get current world position of the camera
        this.el.object3D.getWorldPosition(AFrameP5.camera_positionWorld);

        // set the camera semaphore (bug fix for immediate calls to 'World.setUserPosition')
        AFrameP5.camera_semaphore = true;

        // get current local position of the camera
        AFrameP5.camera_positionLocal = this.el.object3D.position;
        ...
        AFrameP5.camera_rotation = new THREE.Euler().setFromQuaternion(this.quaternion, this.el.object3D.rotation.order);
    }
});
...
class AFrameP5 {
    ...
    static camera_nudge_values = {
        x: 0,
        y: 0,
        z: 0
    };
    static camera_nudge_dirty = false;

    // internal reference to the user-created World instance
    static world_instance_reference = undefined;
    ...
    static World = class {
        ...
        constructor(id, mouseOrGaze = 'mouse') {
            console.log("A-FrameP5 v2.3 (Craig Kapp, 2017-2024)");
            console.log("Documentation: https://cs.nyu.edu/~kapp/aframep5");

            // store a reference to the user-created World instance
            // there should be only one of these per HTML document
            AFrameP5.world_instance_reference = this;
            ...
            // set up our camera
            this.camera = new AFrameP5.Camera(mouseOrGaze, this.scene);
            this.scene.appendChild(this.camera.holder);

            // allow the use of WASD keys for navigation
            this.wasd = true;
            this.wasdFly = true;

            // setup default WASD controls (enabled, flying, acceleration of 65)
            this.setWASD({ acceleration: 65, enabled: true, fly: true });
            ...
            // heads up display (HUD) control values zzz
            this.HUD = {
                distance: -1,
                entities: []
            }
            ...
            var _interval = setInterval(function () {
                ...
                // ensure that our camera is active (hack, fix later)
                _this.camera.cameraEl.setAttribute('camera', 'active: true');
                ...
                AFrameP5.updateDynamicTextures();
                ...
                if (_this.setUserPositionEarly && AFrameP5.camera_semaphore) {
        ...
        setWASD(v) {
            const props = [];
            if (v.acceleration) {
                this.wasdAcceleration = v.acceleration;
            }
            props.push(`acceleration: ${this.wasdAcceleration}`);
            if (v.enabled || v.enabled == false) {
                this.wasdEnabled = v.enabled;
            }
            props.push(`enabled: ${this.wasdEnabled}`);

            if (v.fly || v.fly == false) {
                this.wasdFly = v.fly;
            }
            props.push(`fly: ${this.wasdFly}`);

            this.camera.cameraEl.setAttribute('wasd-controls', props.join(';'));
        }
        ...
        enableWASD(v) {
            this.setWASD({ enabled: v });
        }
        setWASDAcceleration(v) {
            this.setWASD({ acceleration: v });
        }
        getWASDAcceleration() {
            return this.wasdAcceleration;
        }
        add(entity) {
        ...
        setUserPosition(x, y, z) {
            ...
            if (AFrameP5.camera_semaphore == false) {
        ...
        getUserRotation() { // zzz
            ...
            return {
                x: AFrameP5.camera_rotation.x,
                y: AFrameP5.camera_rotation.y,
                z: AFrameP5.camera_rotation.z
            };
        }
        ...
        moveUserForward(d) {
            ...
            let vectorCamera = new THREE.Vector3();
            vectorCamera.setFromMatrixPosition(this.camera.cameraEl.object3D.matrixWorld);

            let xDiff = vectorHUD.x - vectorCamera.x;
            let yDiff = vectorHUD.y - vectorCamera.y;
            let zDiff = vectorHUD.z - vectorCamera.z;
        ...
        moveUserBackward(d) {
            this.moveUserForward(-d);
        }

        moveUserLeft(d) {
            let vectorHUD = new THREE.Vector3();
            vectorHUD.setFromMatrixPosition(this.camera.cursorLeft.tag.object3D.matrixWorld);
        ...
        moveUserUp(d) {
            let vectorHUD = new THREE.Vector3();
            vectorHUD.setFromMatrixPosition(this.camera.cursorUp.tag.object3D.matrixWorld);
            ...
            // note that up/down motion will ignore the flying flag
            this.camera.nudgePosition(xDiff * d, yDiff * d, zDiff * d);
        }      
        ...
        teleportToObject(element) {
            this.setUserPosition(element.getX(), element.getY(), element.getZ());
        }
        ...
        isControllerRightTriggerDown() {
            return AFrameP5.controller_rightTriggerDown;
        }

        isControllerLeftTriggerDown() {
            return AFrameP5.controller_leftTriggerDown;
        }

        isControllerRightGripDown() {
            return AFrameP5.controller_rightGripDown;
        }

        isControllerLeftGripDown() {
            return AFrameP5.controller_leftGripDown;
        }

        isControllerAButtonDown() {
            return AFrameP5.controller_aButtonDown;
        }

        isControllerBButtonDown() {
            return AFrameP5.controller_bButtonDown;
        }

        isControllerXButtonDown() {
            return AFrameP5.controller_xButtonDown;
        }

        isControllerYButtonDown() {
            return AFrameP5.controller_yButtonDown;
        }

        getControllerRightThumbstickDirection() {
            return AFrameP5.controller_rightThumbstickDirection;
        }

        getControllerLeftThumbstickDirection() {
            return AFrameP5.controller_leftThumbstickDirection;
        }

        getControllerRightThumbstickRawData() {
            return AFrameP5.controller_rightThumbstickRawData;
        }

        getControllerLeftThumbstickRawData() {
            return AFrameP5.controller_leftThumbstickRawData;
        }
        ...
        addToHUD(entity, x, y, z) {

            // set the entity's position in the HUD
            if ((x || x == 0) && (y || y == 0) && (z || z == 0)) {
                entity.setPosition(x, y, z);
            }

            // only set the entity's x & y position in the HUD, using the HUD distance for the z position
            else if ((x || x == 0) && (y || y == 0)) {
                entity.setPosition(x, y, this.HUD.distance);
            }

            // add the entity to the HUD
            this.camera.cameraEl.appendChild(entity.tag);

            // store a reference to this entity
            this.HUD.entities.push(entity);
        }

        removeFromHUD(entity) {
            // remove the entity from the HUD
            this.camera.cameraEl.removeChild(entity.tag);

            // remove the entity from our internal reference array
            const index = this.HUD.entities.indexOf(entity);
            if (index > -1) {
                this.HUD.entities.splice(index, 1);
            }
        }

        getHUDDistance() {
            return this.HUD.distance;
        }

        setHUDDistance(d) {
            this.HUD.distance = d;
            for (let entity of this.HUD.entities) {
                entity.setZ(d);
            }
        }
    ...
    static Camera = class {
        ...
        constructor(mouseOrGaze, threeScene) {
            ...
            // construct our cursor graphic, which will also serve as a landmark entity to move the user forward / backward
            this.cursor = new AFrameP5.Ring({
                x: 0,
                y: 0,
                z: -1.0,
                radiusInner: 0.02,
            ...
            // construct additional invisible landmarks to handle left / right and up / down motion
            this.cursorLeft = new AFrameP5.Plane({
                x: -1, y: 0, z: 0,
                width: 0.1, height: 0.1,
                opacity: 0.0
            });
            this.cursorUp = new AFrameP5.Plane({
                x: 0, y: 1, z: 0,
                width: 0.1, height: 0.1,
                opacity: 0.0
            });

            // add ring indicator and other landmarks to our entity holder
            this.cameraEl.appendChild(this.cursor.tag);
            this.cameraEl.appendChild(this.cursorLeft.tag);
            this.cameraEl.appendChild(this.cursorUp.tag);
        ...
        // 2024-11-27: updated to utilize the nudgePosition method, which hooks into the centralized object that handles all repositioning requests
        setPosition(x, y, z) {
            // compute how much of a nudge this movement involves
            let nudgeX = x - AFrameP5.camera_positionWorld.x;
            let nudgeY = y - AFrameP5.camera_positionWorld.y;
            let nudgeZ = z - AFrameP5.camera_positionWorld.z;

            this.nudgePosition(nudgeX, nudgeY, nudgeZ);
        }

        // 2024-11-27: nudging the position now updates a centralized object which will be referenced by the rotation-and-position-reader component
        // this allows for a centralized change of position, supporting multiple simultaneous key presses when implementing custom motion systems
        nudgePosition(x, y, z) {
            // update centralized object which will force the camera to update its position in its tick function
            AFrameP5.camera_nudge_values.x += x;
            AFrameP5.camera_nudge_values.y += y;
            AFrameP5.camera_nudge_values.z += z;
            AFrameP5.camera_nudge_dirty = true;
        }

        // unused
        nudgePositionOld(x, y, z) {
            // only need to update THREE.JS object
            this.holder.object3D.position.x = AFrameP5.camera_positionWorld.x + x;		// this works, but breaks when WASD is used
            this.holder.object3D.position.y = AFrameP5.camera_positionWorld.y + y;
            this.holder.object3D.position.z = AFrameP5.camera_positionWorld.z + z;
        ...
        getX() {
            return AFrameP5.camera_positionWorld.x;
        }

        getY() {
            return AFrameP5.camera_positionWorld.y;
        }

        getZ() {
            return AFrameP5.camera_positionWorld.z;
        }
    ...
    static Box = class {
        ...
        constructor(opts) {
            ...
            AFrameP5.setEntityOptions(opts, this);
            ...
            // set geometry
            AFrameP5.setGeometry(this);

            // set material
            AFrameP5.processMaterial(this);
            AFrameP5.setMaterial(this);

            // set scale
            AFrameP5.setScale(this.opts, this);

            // set position
            AFrameP5.setPosition(this.opts, this);

            // set rotation
            AFrameP5.setRotation(this.opts, this);

            // set visibility
            AFrameP5.setVisibility(this.opts, this);

            // set click handler
            AFrameP5.setClickHandlers(this);

            // init common setters / getters
            AFrameP5.initializerSettersAndGetters(this);
        }
    ...
    static Light = class {
        ...
        constructor(opts) {
            ...
            // process light opts
            AFrameP5.setLight(this.opts, this);
    ...
    static Sky = class {
        ...
        constructor(opts) {
            ...
            if ('dynamicTexture' in opts && opts.dynamicTexture == true) {
                ...
                AFrameP5.registerDynamicTexture(this);
            } else {
            ...
            this.setRadius = function (r) {
                if ('radius' in this) {
                    this.radius = r;
                    AFrameP5.processSkyChanges(this);
                }
            }
    ...
    static setTextOptions(entity) {
        ...
        entity.setText = function (t) {
            this.text = t;
            AFrameP5.processTextChanges(this);
        }
        ...
        entity.lookAt = function (otherEntity) {
            this.tag.object3D.lookAt(otherEntity.tag.object3D.position);
        }

        AFrameP5.processTextChanges(entity);
    }
    ...
    static setClickHandlers(entity) {
        ...
        if ('tickFunction' in entity.opts) {
            entity.tickFunction = entity.opts.tickFunction;
            entity.tag.eRef = entity;
            entity.tag.setAttribute('generic-interaction-handler', '');
        }
    ...
    static setEntityOptions(opts, entity) {
        ...
        // setup a "customData" object for storing custom data
        if (opts.customData) {
            entity.customData = opts.customData;
        }
        else {
            entity.customData = {};
        }
    ...
    static processMaterial(entity) {
        ...
        if ('dynamicTexture' in opts && opts.dynamicTexture == true) {
            ...
            AFrameP5.registerDynamicTexture(entity);
        } else {
    ...
    static initializerSettersAndGetters(entity) {
        ...
        // force an entity to look at the camera
        entity.lookAtUser = function() {
            const worldPosition = new THREE.Vector3();
            AFrameP5.world_instance_reference.camera.cameraEl.object3D.getWorldPosition(worldPosition);
            this.tag.object3D.lookAt(worldPosition);
        }

    }

    static registerDynamicTexture(entity) {
        AFrameP5.entitiesUsingDynamicTextures.push(entity)
    }

    static updateDynamicTextures() {
        for (let i = 0; i < AFrameP5.entitiesUsingDynamicTextures.length; i++) {
            AFrameP5.entitiesUsingDynamicTextures[i].updateTexture();
        }
    }
